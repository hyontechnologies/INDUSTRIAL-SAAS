...
-- Partition also by tenant for future multi-node scalability
-- (add space partitioning only when > 3 tenants with heavy load)
-- SELECT add_dimension('telemetry_raw', 'tenant_id', number_partitions => 4);
...
-- Compression for older alarm history
ALTER TABLE alarms SET (
    timescaledb.compress,
    timescaledb.compress_segmentby = 'tenant_id, plant_id, severity',
...
-- ─────────────────────────────────────────────────────────────────────────────
-- The FastAPI layer enforces tenant_id from JWT/API key.
-- RLS is a defence-in-depth layer for Grafana's native PG datasource
-- and any direct psql access.

-- Create a low-privilege application role for FastAPI + Grafana
DO $$ BEGIN
...
-- Application user (used by FastAPI asyncpg pool)
DO $$ BEGIN
...
-- Grant table permissions to historian_app role
...
-- Grant read-only to grafana_grafana role
GRANT SELECT ON
...
-- ── RLS policies: rows visible only when current_setting matches tenant_id ───
--  FastAPI sets: SET app.current_tenant = '<tenant_id>' on each connection
--  (Add this to _init_connection if you want RLS enforced at DB level too.
--   For MVP, FastAPI WHERE clauses are the primary isolation layer.)

CREATE POLICY telemetry_raw_tenant ON telemetry_raw
...
-- historian_app bypasses RLS for admin operations
ALTER TABLE telemetry_raw    FORCE ROW LEVEL SECURITY;
ALTER TABLE telemetry_latest FORCE ROW LEVEL SECURITY;
-- Note: if you want FastAPI to bypass RLS, GRANT BYPASSRLS to historian_user
-- For MVP, keep FastAPI using WHERE tenant_id=$1 (RLS is defence-in-depth only)

-- ─────────────────────────────────────────────────────────────────────────────
-- §12  LOW-MEMORY TUNING FOR 4 GB RAM VM
--      Apply these in postgresql.conf (or via ALTER SYSTEM)
--      Requires: SELECT pg_reload_conf(); after ALTER SYSTEM
-- ─────────────────────────────────────────────────────────────────────────────
-- ALTER SYSTEM SET shared_buffers              = '512MB';    -- 1/8 of RAM
-- ALTER SYSTEM SET effective_cache_size        = '1536MB';   -- 3/8 of RAM
-- ALTER SYSTEM SET maintenance_work_mem        = '128MB';
-- ALTER SYSTEM SET work_mem                    = '16MB';     -- per sort/hash
...
--      Replace with your actual values or remove for production
-- ─────────────────────────────────────────────────────────────────────────────
